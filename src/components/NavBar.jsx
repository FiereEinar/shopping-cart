...
export default function NavBar({ shoppingItems }) {
  ...
  return (
    <nav
      id="nav"
      className="flex flex-col md:flex-row justify-between items-center p-2 flex-wrap gap-2 bg-white rounded px-5 shadow"
    >
      ...
      <div className="flex-1 flex justify-end items-center w-full">
        ...
        >
          ...
          {cartQuantity > 0 && (
            <span className="absolute bg-orange-400 rounded-full w-4 h-4 text-white top-0 flex justify-center items-center right-[-8px] text-[0.8rem]">
              {cartQuantity}
            </span>
          )}
        </button>
